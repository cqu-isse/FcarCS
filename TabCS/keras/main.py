...
class CodeSearcher:
    ...
    def train(self, model):
        ...
        f1=open('/data/dengzhongyang/TabCS/keras/results/training_results.txt','a',encoding='utf-8',errors='ignore')
        for i in range(self.train_params['reload']+1, nb_epoch):
            ...
            hist = model.fit([chunk_padded_methnames,chunk_padded_apiseqs,chunk_padded_tokens,chunk_padded_sbt, chunk_padded_good_descs, chunk_padded_bad_descs], epochs=1, batch_size=batch_size, validation_split=split,callbacks=[early_stopping])
            #hist = model.fit([chunk_padded_methnames,chunk_padded_apiseqs,chunk_padded_sbt, chunk_padded_good_descs, chunk_padded_bad_descs], epochs=1, batch_size=batch_size, validation_split=split,callbacks=[early_stopping])
    ...
    def valid(self, model,K):
        ...
        for i in range(data_len):
            ...
            #sims = model.predict([methnames,apiseqs,sbt,descs], batch_size=data_len).flatten()
    ...
    def eval(self, model, K):
        ...
        for i in range(data_len):
            ...
            n_results = K          
            sims = model.predict([methnames,apiseqs,tokens,sbt,descs], batch_size=800).flatten()
            #sims = model.predict([methnames,apiseqs,sbt,descs], batch_size=1000).flatten()
        ...
        f2=open('/data/dengzhongyang/TabCS/keras/results/eval_results.txt','a',encoding='utf-8',errors='ignore')
        f2.write('SuccRate1={},SuccRate5={},SuccRate10={}, ACC={}, MRR={}, MAP={}, nDCG={}'.format(succrate1,succrate5,succrate10,acc,mrr,map,ndcg))
        return succrate1,succrate5,succrate10,acc,mrr,map,ndcg
    ...
    def repr_code(self,model):
        ...
        vecs=model.repr_code([methnames, apiseqs, tokens], batch_size=2000)     #model.predict()方法可以设置batchsize来分批次分配tensor
    ...
    def search(self,model,query,n_results=10):
        ...
        sims = model.predict([padded_methnames,padded_apiseqs,padded_tokens,padded_sbt,padded_desc],batch_size=1000).flatten()         #是否需要加batchsize ?
...
if __name__ == '__main__':
    K.set_session(get_session(1.0))  # using 80% of total GPU Memory
    args = parse_args()
    ...
    if args.mode=='train':  
        ...
        # codesearcher.load_code_reprs()
        codesearcher.load_codebase()            #把raw code 存至一个list里面
        resultsFile=codecs.open('/data/dengzhongyang/TabCS/keras/results/search_results.txt','w',encoding='utf-8',errors='ignore')
        queriesFile=codecs.open('/data/dengzhongyang/TabCS/keras/results/query.txt', "r", encoding='utf-8',errors='ignore')
        n_results=10
        while 1:
