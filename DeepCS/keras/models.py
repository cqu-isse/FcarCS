...
from keras.layers import Input, Concatenate, Dot, Embedding, Dropout, Lambda, Activation, LSTM, Dense
...
class JointEmbeddingModel:
    ...
    def build(self):
        ...
        #2.rnn
        f_rnn = LSTM(self.model_params.get('n_lstm_dims', 200), recurrent_dropout=0.2, 
                     return_sequences=True, name='lstm_methname_f')
        
        b_rnn = LSTM(self.model_params.get('n_lstm_dims', 200), return_sequences=True, 
        ...
        #1.embedding
        init_emb_weights = np.load(self.model_params['init_embed_weights_api']) if self.model_params['init_embed_weights_api'] is not None else None
        if init_emb_weights is not None: init_emb_weights = [init_emb_weights]
        ...
        #2.rnn
        f_rnn = LSTM(self.model_params.get('n_lstm_dims', 200), return_sequences=True, recurrent_dropout=0.2,
                      name='lstm_apiseq_f')
        b_rnn = LSTM(self.model_params.get('n_lstm_dims', 200), return_sequences=True, recurrent_dropout=0.2, 
