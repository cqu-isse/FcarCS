...
class CodeSearcher:
    ...
    def load_use_data(self):
        ...
        return methodname,apiseqs,tokens      
    ...
    def train(self, model):
        ...
        f1=open('/data/dengzhongyang/DeepCS/keras/results/training_results.txt','a',encoding='utf-8',errors='ignore')
        for i in range(self.train_params['reload']+1, nb_epoch):
            print('Epoch %d :: \n' % i, end='')            
            logger.debug('loading data chunk..')
            chunk_methnames,chunk_apiseqs,chunk_tokens,chunk_descs =self.load_training_data_chunk()
            ...
            hist = model.fit([chunk_padded_methnames,chunk_padded_apiseqs,chunk_padded_tokens, chunk_padded_good_descs, chunk_padded_bad_descs], epochs=1, batch_size=batch_size, validation_split=split,callbacks=[early_stopping])
    ...
    def eval(self, model, K):
        ...
        param:
            poolsize - size of the code pool, if -1, load the whole test set
        """
        def SUCCRATE10(real,predict,n_results):
            sum=0.0
            for val in real:
        ...
        def SUCCRATE5(real,predict,n_results):
            sum=0.0
            for val in real:
                try: index=predict.index(val)
                except ValueError: index=-1
            ...
            return sum/float(len(real))
        def SUCCRATE1(real,predict,n_results):
            sum=0.0
            for val in real:
                try: index=predict.index(val)
                except ValueError: index=-1
        ...
        if self._eval_sets is None:
            ...
            self._eval_sets['descs']=descs
        data_len=len(self._eval_sets['descs'])
        numbers = codecs.open('/data/dengzhongyang/DeepCS/keras/results/bootstrap_nums.txt','a',errors = 'ignore', encoding = 'utf-8')
        for k in range(0,10):
            succrate10,succrate5,succrate1,acc,mrr,map,ndcg=0,0,0,0,0,0,0
            ...
            for number in bootstrap_num_list:
                ...
                real=[num]
                succrate10+=SUCCRATE10(real,predict_origin,n_results)
                succrate5+=SUCCRATE5(real,predict_origin,n_results)
                succrate1+=SUCCRATE1(real,predict_origin,n_results)
            ...
            print('SuccRate10={},SuccRate5={},SuccRate1={}, ACC={}, MRR={}, MAP={}, nDCG={}'.format(succrate10,succrate5,succrate1,acc,mrr,map,ndcg))
            f2=codecs.open('/data/dengzhongyang/DeepCS/keras/results/bootsrtap_results.txt','a',encoding='utf-8',errors='ignore')
            f2.write('SuccRate10={},SuccRate5={},SuccRate1={}, ACC={}, MRR={}, MAP={}, nDCG={}'.format(succrate10,succrate5,succrate1,acc,mrr,map,ndcg) + '\n')
    ...
    def repr_code(self,model):
        ...
        vecs=model.repr_code([methnames, apiseqs, tokens], batch_size=1000)     #model.predict()方法可以设置batchsize来分批次分配tensor
    ...
    def search(self,model,query,n_results=10):
        ...
        sims = model.predict([padded_methnames,padded_apiseqs,padded_tokens,padded_desc],batch_size=1000).flatten()         #是否需要加batchsize ?
...
if __name__ == '__main__':
    K.set_session(get_session(0.3))  # using 80% of total GPU Memory
    args = parse_args()
    ...
    if args.mode=='train':  
        ...
        resultsFile=codecs.open('/data/dengzhongyang/DeepCS/keras/results/search_results.txt','w',encoding='utf-8',errors='ignore')
        queriesFile=codecs.open('/data/dengzhongyang/DeepCS/keras/results/query.txt', "r", encoding='utf-8',errors='ignore')
        n_results=10
        while 1:
