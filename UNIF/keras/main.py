...
class CodeSearcher:
    ...
    def load_training_data_chunk(self):
        ...
        return chunk_tokens,chunk_descs    
    def load_valid_data_chunk(self):
        ...
        return chunk_tokens[:500],chunk_descs[:500]   
    ...
    def train(self, model):
        ...
        f1=open('/data/dengzhongyang/UNIF/keras/results/training_results.txt','a',encoding='utf-8',errors='ignore')
        for i in range(self.train_params['reload']+1, nb_epoch):
            print('Epoch %d :: \n' % i, end='')            
            logger.debug('loading data chunk..')
            chunk_tokens,chunk_descs =self.load_training_data_chunk()
            ...
            hist = model.fit([chunk_padded_tokens, chunk_padded_good_descs, chunk_padded_bad_descs], epochs=1, batch_size=batch_size, validation_split=split,callbacks=[early_stopping])
    ...
    def valid(self, model,K):
        ...
        for i in range(data_len):
            ...
            sims = model.predict([tokens,descs], batch_size=data_len).flatten()
    ...
    def eval(self, model, K):
        ...
        #load valid dataset
        if self._eval_sets is None:
            tokens,descs=self.load_test_data_chunk()
            self._eval_sets=dict()
            ...
            self._eval_sets['descs']=descs
        data_len=len(self._eval_sets['descs'])
        numbers = codecs.open('/data/dengzhongyang/UNIF/keras/results/bootstrap_nums.txt','a',errors = 'ignore', encoding = 'utf-8')
        for k in range(0,10):
            ...
            for number in bootstrap_num_list:
                ...
                n_results = K          
                sims = model.predict([tokens,descs], batch_size=1000).flatten()
            ...
            f2=codecs.open('/data/dengzhongyang/UNIF/keras/results/bootsrtap_results.txt','a',encoding='utf-8',errors='ignore')
    ...
    def repr_code(self,model):
        ...
        vecs=model.repr_code([tokens], batch_size=1000)     #model.predict()方法可以设置batchsize来分批次分配tensor
    ...
    def search(self,model,query,n_results=10):
        ...
        sims = model.predict([padded_tokens,padded_desc],batch_size=1000).flatten()         #是否需要加batchsize ?
...
if __name__ == '__main__':
    K.set_session(get_session(0.5))  # using 80% of total GPU Memory
    args = parse_args()
    ...
    if args.mode=='train':  
        ...
        resultsFile=codecs.open('/data/dengzhongyang/UNIF/keras/results/search_results.txt','w',encoding='utf-8',errors='ignore')
        queriesFile=codecs.open('/data/dengzhongyang/UNIF/keras/results/query.txt', "r", encoding='utf-8',errors='ignore')
        n_results=10
        while 1:
